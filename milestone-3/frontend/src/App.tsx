  ...
  return (
    <BrowserRouter>
      ...
      <Routes>
        ...
        {
          recipes.map((rec) => (<Route path = {"recipe/" + rec.name} element = {<RecipePage {...rec} ext = {false} />} />))
        }
        {/*An array of routes created based on each external recipe from the Hu's Chew's API:*/}
        {
          externalRecipes.map((rec) => (<Route path = {"externalRecipe/" + rec.name} element = {<RecipePage {...rec} ext = {true}/>} />))
        }
      </Routes>
    </BrowserRouter>
  );
